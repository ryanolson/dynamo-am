...
use crate::api::{
    client::{ActiveMessageClient, Endpoint, PeerInfo, WorkerAddress},
    handler::{ActiveMessage, HandlerId, InstanceId},
};
use crate::protocols::{ControlMetadata, receipt::ReceiptAck};
use crate::runtime::response_manager::SharedResponseManager;
...
impl ActiveMessageClient for NetworkClient {
    ...
    async fn discover_peer(&self, endpoint: &str) -> Result<PeerInfo> {
        use crate::protocols::responses::DiscoverResponse;
    ...
    async fn await_handler(
        ...
        let mut builder = self
            ...
            .payload(payload)?
            .expect_response::<crate::protocols::responses::WaitForHandlerResponse>();
        ...
        let response: crate::protocols::responses::WaitForHandlerResponse =
    ...
    async fn list_handlers(&self, instance_id: InstanceId) -> Result<Vec<HandlerId>> {
        ...
        let status = self
            ...
            .expect_response::<crate::protocols::responses::ListHandlersResponse>()
            .send(instance_id)
            .await?;

        debug!("list_handlers: Message sent, waiting for response");
        let response: crate::protocols::responses::ListHandlersResponse =
            status.await_response().await?;
        debug!(
