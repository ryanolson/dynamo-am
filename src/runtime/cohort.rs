...
impl LeaderWorkerCohort {
    ...
    pub async fn register_handlers(
        ...
        use crate::protocols::responses::JoinCohortResponse;
    ...
    pub async fn broadcast_to_workers_with_acks(
        ...
        for worker_id in workers {
            ...
            let message = crate::handler::ActiveMessage::new(
                Uuid::new_v4(),
                handler.to_string(),
                client.instance_id(),
                payload.clone(),
        ...
        R: for<'a> Deserialize<'a> + Send + 'static,
    {
        ...
        for (rank, worker_id) in workers {
            ...
            let task = tokio::spawn(async move {
                ...
                let mut control = ControlMetadata::fire_and_forget();
                control.set_response_context(message_id, Some(handler.clone()), false);
                        ...
                        if now.duration_since(last_hb) > heartbeat_timeout { // 2 minutes
                            ...
                            tokio::spawn({
                                ...
                                async move {
                                    ...
                                    let message_id = Uuid::new_v4();
                                    let mut control = ControlMetadata::fire_and_forget();
                                    control.set_response_context(
                                        message_id,
                                        Some("_health_check".to_string()),
                                        false,
                                    );
                                    let message = ActiveMessage::new(
                                        message_id,
